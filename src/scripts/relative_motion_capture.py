...
from util.VideoStream import VideoStream, VideoStreamCalibrated
...
if __name__ == "__main__":
    ...
    while i < 100:
        ...
        vicon_obj.get_frame()
        cam_state = vicon_obj.get_object_state('webcam')
        obj_state = vicon_obj.get_object_state('rel-motion-obj')
    ...
    for i in range(0, len(data.camera_state)):
        ...
        for a in data.camera_state[i].state.translation: row.append(a)
        for a in data.object_state[i].state.translation: row.append(a)
        for a in data.camera_state[i].state.quaternion: row.append(a)
        for a in data.object_state[i].state.quaternion: row.append(a)
